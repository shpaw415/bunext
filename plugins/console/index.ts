...
export const BuildServerComponentWithHooksWarning = [
    separator,
...
const clear = "\x1b[0m";
export function ToColor<T extends string | number>(color: string, value: T): T {
    try {
        return Bun.color(color, "ansi") + value?.toString() + clear as T;
    } catch {
        return value as T;
    }
}
...
class ScrollingConsole {
    ...
    init() {
        if (process.env?.__DISABLE_DEV_CONSOLE_ == "true" || this.is_inited) return;
        this.is_inited = true;
        terminal.grabInput({
            mouse: "button",
            safe: false
...
export function redirectConsole(IPCHelper: ClientIPCManager<"cluster">) {
    ...
    console.log = (...args: any[]) => {
        IPCHelper.actions.main.log("log", ...args);
    };

    // Redirect console.error
    console.error = (...args: any[]) => {
        IPCHelper.actions.main.log("error", ...args);
    };

    // Redirect console.warn
    console.warn = (...args: any[]) => {
        IPCHelper.actions.main.log("warn", ...args);
    };

    // Redirect console.info
    console.info = (...args: any[]) => {
        IPCHelper.actions.main.log("info", ...args);
    };

    // Redirect console.debug
    console.debug = (...args: any[]) => {
        IPCHelper.actions.main.log("debug", ...args);
    };

    // Return a function to restore original console methods if needed
    return () => {
...
export default {
    ...
    serverStart: {
        main(ipc) {
            ...
            globalThis.__BUNEXT__CONSOLE__.init();
            ipc.onMessage<consoleLogMessage>("log-to-console", ({ log, args }) => {
                console[log as "log"](...args);
            });
        },
        cluster(ipc) {
            redirectConsole(ipc);
