 ...
 */
export interface SessionOptions<T extends Record<string, unknown> = {}> {
    data?: SessionData<T>;
...
export class BunextSession<DataType extends Record<string, unknown> = {}> {
    ...
    private _internalData: SessionData<DataType> = {
        public: {} as DataType,
        private: {} as DataType,
    };
    ...
    init(data: SessionData<DataType> | null): void {
    ...
    private _updateTimeout(newTimeout: number) {
        this._sessionTimeout = newTimeout;
    }
    /**
     ...
     */
    getData<DataAwaited extends keyof SessionData<DataType> = "public">(): SessionData<DataType>[DataAwaited] | undefined {
        try {
...
export function GetSessionFromResponse(response: Response) {
    const sessionHeader = response.headers.get(NewSessionCookieName);
